/**
 * Projection from the shared managed-process handle to the Qoder Agent SDK's
 * custom-spawn process interface (`spawnQoderCLIProcess`).
 *
 * @module dsh-subagent-qoder/process
 */
...
/**
 * Translate one SDK spawn request to the shared process owner.
 ...
 */
export function qoderSpawnSpec(
  options: SpawnOptions,
  graceMs: number,
): SubprocessSpawnSpec {
  if (options.cwd === undefined || options.cwd.length === 0) {
    throw new Error('subagent-qoder: SDK spawn request omitted its workspace')
  }
  return {
 ...
 * in the Qoder SDK; this adapter only projects streams and exit events.
 */
export class ManagedQoderProcess implements SpawnedProcess {
  readonly stdin
  readonly stdout
